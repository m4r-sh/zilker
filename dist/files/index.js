...
function Assets({
  ...
  let FileModel = Model(File, {
    ...
    get asset_route() {
      ...
      return is_generated ? name2 : name2 + route.substring(route.lastIndexOf("."));
    },
    async update() {
...
async function Pages({
  ...
  let FileModel = Model(Script2, {
    ...
    get output_html_route() {
      let { name: name2 } = this;
      return name2.match(/[\/]*index/) ? `${name2}.html` : `${name2}/index.html`;
    },
    get used_classes() {
  ...
  return Model(Build, {
    ...
    async prompt_create({ Text: Text4, List }) {
      let page_name = await Text4({
        message: "Page name?"
      });
...
function Views({
  ...
  return Model(Build, {
    ...
    async prompt_create({ Text: Text4, List }) {
      let component_name = await Text4({
